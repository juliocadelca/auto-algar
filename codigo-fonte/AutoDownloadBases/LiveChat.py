...
def LiveChat():
    ...
    try:
        Aguarde_a_Imagem(AguardeLiveChat) # Aguardando a pagina carregar por completo
    except SaidaException:
        return
    
    localizar_e_clicar(DataEntrada) # Clicando no data_inicio para filtrar data
    ...
    localizar_e_clicar(CaixaNãoFlegadaLiveChat) # Clicando na caixa de selção novamente para selecionar todos do mês atual
    ...
    localizar_e_clicar(TresPontinhosDois) # Clicando nos 3 potinhos
    ...
    for num in lista:
        if mes_atual == num:
            caminho = (rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\CHAT\CHAT_LOOKER\{ano_atual_str}-0{mes_atual_str}-chat")
        else:
            caminho = (rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\CHAT\CHAT_LOOKER\{ano_atual_str}-{mes_atual_str}-chat")
