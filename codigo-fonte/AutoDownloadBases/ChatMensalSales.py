...
def ChatMensalSales():
    ...
    py.click(x=133, y=610), py.sleep(1) # Clicando na posição Chat Mensal
    ...
    py.click(x=1197, y=744), py.sleep(25)
    lista = [1,2,3,4,5,6,7,8,9]
    if mes_atual in lista:
        caminho = (rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\CHAT\CHAT_LOOKER\{ano_atual_str}-0{mes_atual_str}-prov_chat_assinc")
    else:
        caminho = (rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\CHAT\CHAT_LOOKER\{ano_atual_str}-{mes_atual_str}-prov_chat_assinc")
    py.typewrite(caminho), py.sleep(2)
    ...
    salvar_mes_atual(mes_atual), py.sleep(5) # Salva o mês atual para a próxima execução
