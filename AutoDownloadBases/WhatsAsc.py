...
def WhatsAsc():
    ...
    localizar_e_clicar(rota.whats_asc_img), py.sleep(1) # Clicando na posição Whats Asc
    ...
    py.typewrite(f"{nova_hora_str}"), py.sleep(0.5)
    ...
    for num in lista:
        if dia_atual and mes_atual == num: # Selecionando pasta de destino - Criar condição para mes com '0'
            caminho = rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\WHATSASC\0{mes_atual_str}-{ano_atual_str}-asc\0{dia_atual_str}-0{mes_atual_str}-{ano_atual_str}-asc"
        elif dia_atual in lista:
            caminho = rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\WHATSASC\{mes_atual_str}-{ano_atual_str}-asc\0{dia_atual_str}-{mes_atual_str}-{ano_atual_str}-asc"
        elif mes_atual in lista:
            caminho = rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\WHATSASC\0{mes_atual_str}-{ano_atual_str}-asc\{dia_atual_str}-0{mes_atual_str}-{ano_atual_str}-asc"
        else:
            caminho = rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\WHATSASC\{mes_atual_str}-{ano_atual_str}-asc\{dia_atual_str}-{mes_atual_str}-{ano_atual_str}-asc"
    ...
    if '09:00' <= hora_atual_str < '10:00':  # Verifica se a hora está entre 9:00 e 9:59
        ...
        py.press('enter')
        localizar_e_clicar(rota.botao_sim)  # Salavando em diferentes horários clicando no botão sim
