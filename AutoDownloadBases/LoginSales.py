...
class Caminhos:
    def __init__(self):
        ...
        base_path = os.path.expanduser(r"~/Documentos/app/img") ### Usando os.path.expanduser para garantir que funcione em qualquer máquina
...
def LoginSales():
    ...
    py.sleep(3)
    py.click(x=199, y=56) 
    py.typewrite(rota.manual_boletim) 
    py.press('enter') # Entrando no Manual do Boletim
    ...
    localizar_e_clicar(rota.login_sales_img), py.sleep(1) # Clicando na posição Login Sales
    localizar_e_clicar(rota.link) # Clicando no link do relatório
    localizar_e_clicar(rota.https), py.sleep(5)

    py.hotkey('ctrl', '+'), py.sleep(1)
    py.click(x=1718, y=102)
    ...
    for num in lista:
        if mes_atual == num:
            caminho = (rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\AGENTESALES\{ano_atual_str}\0{mes_atual_str}-{ano_atual_str}-agtsales")
            break
        else:
            caminho = (rf"G:\CSC\Atendimento\Algar\Planejamento\01.BOLETIM\BASES\AGENTESALES\{ano_atual_str}\{mes_atual_str}-{ano_atual_str}-agtsales")
